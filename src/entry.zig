...
const entry_type: c.rb_data_type_t = .{
    ...
    .function = .{ .dmark = @ptrCast(&markEntry), .dfree = @ptrCast(&freeEntry), .dsize = sizeEntry },
    .data = null,
...
fn sizeEntry(raw: ?*const anyopaque) callconv(.c) usize {
...
pub fn createEntry(archive_rb: c.VALUE, archive: *c.zip_t, index: c.zip_uint64_t, stat: *const c.zip_stat_t, is_dir: bool) c.VALUE {
    ...
    // Wrap before allocating anything the struct will keep: until markEntry
    // can find them, a VALUE stored here is invisible to the GC.
    const obj = c.TypedData_Wrap_Struct(entry_class, &entry_type, entry);

    const name: [*:0]const u8 = stat.name orelse "";
    entry.name_rb = c.rb_utf8_str_new_cstr(name);
    ...
    entry.comment_rb = convertComment(comment_ptr, comment_length);
...
fn entrySize(self: c.VALUE) callconv(.c) c.VALUE {
    ...
    if (entry.valid & c.ZIP_STAT_SIZE == 0) return c.Qnil;
    return c.ULL2NUM(entry.size);
}
fn entryCompressedSize(self: c.VALUE) callconv(.c) c.VALUE {
    const entry = getEntry(self);
    if (entry.valid & c.ZIP_STAT_COMP_SIZE == 0) return c.Qnil;
    return c.ULL2NUM(entry.comp_size);
}
...
fn entryTime(self: c.VALUE) callconv(.c) c.VALUE {
    const entry = getEntry(self);
    if (entry.valid & c.ZIP_STAT_MTIME == 0) return c.Qnil;
    return c.rb_funcall(c.rb_cTime, c.rb_intern("at"), 1, c.TIMET2NUM(entry.mtime));
}

fn entryCompressionMethod(self: c.VALUE) callconv(.c) c.VALUE {
    const entry = getEntry(self);
    if (entry.is_dir) return c.INT2NUM(c.ZIP_CM_STORE);
    if (entry.valid & c.ZIP_STAT_COMP_METHOD == 0) return c.Qnil;
    return c.INT2NUM(@as(c_int, entry.comp_method));
}

fn entryEncryptionMethod(self: c.VALUE) callconv(.c) c.VALUE {
    const entry = getEntry(self);
    if (entry.valid & c.ZIP_STAT_ENCRYPTION_METHOD == 0) return c.Qnil;
    return c.UINT2NUM(entry.encryption_method);
}

fn entryEncrypted(self: c.VALUE) callconv(.c) c.VALUE {
    const entry = getEntry(self);
    if (entry.valid & c.ZIP_STAT_ENCRYPTION_METHOD == 0) return c.Qfalse;
    return if (entry.encryption_method != c.ZIP_EM_NONE) c.Qtrue else c.Qfalse;
}
...
fn inspectEntry(self: c.VALUE) callconv(.c) c.VALUE {
    const entry = getEntry(self);
    const result = c.rb_str_new_cstr("#<LibZip::Entry name=");
    _ = c.rb_str_append(result, c.rb_inspect(entry.name_rb));
    _ = c.rb_str_catf(result, " size=%llu>", entry.size);
    return result;
}
...
fn convertComment(ptr: [*c]const u8, length: c.zip_uint32_t) c.VALUE {
    ...
    const result = c.rb_str_new(ptr, cast.commentBytes(length));
...
test "nameIsDirectory: empty name does not panic" {
    // libzip passes a NULL name for a slot we can't read; createEntry turns
...
test "attributesAreDirectory: UNIX reads the st_mode from the high bits" {
    ...
    // The DOS bit has no meaning when opsys reports UNIX.
